...
contract ERC721Staking is Ownable, ReentrancyGuard {
    ...
    function stake(uint256[] calldata _tokenIds) external nonReentrant {
        // Pre: Calculate unclaimed rewards
        if (stakers[msg.sender].amountStaked > 0) {
            uint256 rewards = calculateRewards(msg.sender);
            stakers[msg.sender].unclaimedRewards += rewards;
        }

        // Add NFTs to staking pool one-by-one
    ...
    function withdraw(uint256[] calldata _tokenIds) external nonReentrant {
        ...
        // Add NFTs back to the staker's account
        uint256 len = _tokenIds.length;
        for (uint256 i; i < len; ++i) {
    ...
    function claimRewards() external {
        ...
        // Trasfer RewardTokens to staker's account
    ...
    function availableRewards(address _user) internal view returns (uint256) {
        require(stakers[_user].amountStaked > 0,
